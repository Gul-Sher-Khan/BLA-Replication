...
def apply_bin_range(
    datalist, enough=None, cohen=0.2, maxBins=16, minBin=4, trivial=1.05
):
    if len(datalist) == 0:
        return datalist
...
def attr_norm(all_elements):
    ...
    """
    if not type(all_elements) is list:
...
def normalize_cols_for_table(table):
    ...
    for col in list(zip(*table)):
        f1, _ = attr_norm(list(col))
        result.append([f1(x) for x in col])
...
def del_col_in_table(list_of_list, col_index):
    ...
    list_of_list = [list(row) for row in zip(*list_of_list)]
    return_table = []
    for index, col in enumerate(list_of_list):
        if index not in col_index:
            return_table.append(col)
    return [list(row) for row in zip(*return_table)]
