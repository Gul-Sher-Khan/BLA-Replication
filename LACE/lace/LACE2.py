...
def add_to_bin(
    ...
    try2add_t = list(zip(*try2add_data_matrix))
    passing_t = list(zip(*passing_bin[1:])) if len(passing_bin) > 2 else []

    for attr in independent_attrs:
        col = try2add_t[attribute_names.index(attr)]
        col = list(map(toolkit.str2num, col))
        my.append(col)

        if len(passing_bin) > 2:
            other_col = passing_t[attribute_names.index(attr)]
            other_col = list(map(toolkit.str2num, other_col))
            others.append(other_col)

    classes = list(try2add_t[attribute_names.index(objective_attr)])
    obj = classes[:]
    other_classes = (
        list(passing_t[attribute_names.index(objective_attr)])
        if len(passing_bin) > 2
        else []
    )
    ...
    my = [list(row) for row in zip(*my)]

    protected_line = copy.deepcopy(my[0])  # saving the data formats!
    others = [list(row) for row in zip(*others)]

    # normalization process
    norm_funcs, denorm_funcs = list(), list()
    for col in [list(row) for row in zip(*(my + others))]:
    ...
    for funi, col in enumerate(my[:-1]):
        uni_my.append([norm_funcs[funi](x) for x in col])
    uni_my.append(my[-1])
    my = [list(row) for row in zip(*uni_my)]

    if len(passing_bin) < 2:
        cache = CLIFF.cliff_core(
            my, cliff_percentage, objective_as_binary, handled_obj=True
        )
    else:
        # normalizing others
        uni_others = list()
        ...
        for funi, col in enumerate(others[:-1]):
            uni_others.append([norm_funcs[funi](x) for x in col])
        uni_others.append(others[-1])
        others = [list(row) for row in zip(*uni_others)]

        to_submits = CLIFF.cliff_core(
            my, cliff_percentage, objective_as_binary, handled_obj=True
        )
        bins = others
    ...
    cache_data = MORPH.simplify_morph(cache_data + others, morph_alpha, morph_beta)[
        : len(cache_data)
    ]
