...
def simplify_morph(data, alpha, beta):
    ...
    """
    classes = list(map(list, zip(*data)))[-1]
    ...
    for row_index, row in enumerate(data):  # for each row
        ...
        boundary_dist = min(
            [toolkit.euclidean_dist(row, data[heg]) for heg in heterogeneous_index]
        )
        boundary_dist /= math.sqrt(len(data[0]) - 2)
        for i in range(len(row)):
            data[row_index][i] += (
                boundary_dist * random.uniform(alpha, beta) * random.choice([1, -1])
            )  # shake
    return data
...
def morph(
    ...
    dataset_t = [list(row) for row in zip(*data_matrix)]
    dataset = list()
    classes = list()
    ...
    dataset = [list(row) for row in zip(*dataset)]
    ...
    is_int = [
        type(i) is int for i in dataset[0]
    ...
    dataset = [list(row) for row in zip(*dataset)]  # transpose.
    norm_funcs = []
    denorm_funcs = []
    ...
    for attr_index, attr_elements in enumerate(dataset):  # for each attribute elements
        ...
        dataset[attr_index] = [f1(x) for x in attr_elements]

    """dataset mode recover..."""
    dataset = [list(row) for row in zip(*dataset)]  # transpose again.
    ...
    for row_index, row in enumerate(dataset):  # for each row
        ...
        boundary_dist = min(
            [toolkit.euclidean_dist(row, dataset[heg]) for heg in heterogeneous_index]
        )
    ...
    for attr_index, attr_elements in enumerate(dataset):  # for each attribute elements
        dataset[attr_index] = [
            denorm_funcs[attr_index](x) for x in attr_elements
        ]  # scale to the original
        for i in range(len(dataset[attr_index])):
            if is_int[attr_index]:
                dataset[attr_index][i] = int(
                    round(dataset[attr_index][i])
                )  # rounding when needed
            else:
                dataset[attr_index][i] = round(dataset[attr_index][i], 4)
    morphed = [
        list(row) for row in zip(*dataset)
    ]  # recover to the original mode and finish.
